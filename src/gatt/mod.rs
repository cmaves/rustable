...
fn match_remote_serv(gdo: &mut RemoteServiceBase, path: &Path) -> 
	Option<(UUID, 
		   ...
		   //U: HasChildren<'b> + GattDbusObject
{
	match path.strip_prefix(gdo.path().file_name().unwrap()) {
		Ok(remaining) => if remaining == Path::new("") {
			...
			for uuid in gdo.get_children() {
				if let Some(matc) = match_remote_char(&mut gdo.get_child(&uuid).unwrap(), remaining) {
...
mod tests {
	...
	use crate::gatt::{LocalDescBase, LocalServiceBase, LocalCharBase, DescFlags, CharFlags};
	use crate::gatt::{match_serv, match_char};
	use crate::ToUUID;
	#[test] 
	fn test_match_char() {
		...
		assert_eq!(match_char(&mut char1, Path::new("char0001")), 
			Some((char1_uuid.clone(), None)));
		assert_eq!(match_char(&mut char1, Path::new("char0001/desc0001")), 
			Some((char1_uuid.clone(), Some(desc1_uuid))));
		assert_eq!(match_char(&mut char1, Path::new("char0002")), 
			None);
		assert_eq!(match_char(&mut char1, Path::new("char0001/desc0002")), 
			None);
	...
	fn test_match_serv() {
		...
		assert_eq!(match_serv(&mut serv1, Path::new("serv0001")), 
			Some((serv1_uuid.clone(), None)));
		assert_eq!(match_serv(&mut serv1, Path::new("char0001")), 
			None);
		assert_eq!(match_serv(&mut serv1, Path::new("serv0001/char0001")), 
			Some((serv1_uuid.clone(), Some((char1_uuid.clone(), None)))));
		assert_eq!(match_serv(&mut serv1, Path::new("serv000001/char0002")), 
			None);
		assert_eq!(match_serv(&mut serv1, Path::new("serv0001/char0001/desc0001")), 
			Some((serv1_uuid.clone(), Some((char1_uuid.clone(), Some(desc1_uuid))))));
		assert_eq!(match_serv(&mut serv1, Path::new("serv0001/char0001/desc0002")), 
			None);
